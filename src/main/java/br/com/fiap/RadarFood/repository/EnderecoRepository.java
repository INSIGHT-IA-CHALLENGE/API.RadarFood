...
public interface EnderecoRepository extends JpaRepository<Endereco, Integer> {
    ...
    @Query("SELECT e FROM Endereco e WHERE e.usuario = :usuario AND e.ativo = true AND " +
       ...
       "UPPER(e.numero) LIKE UPPER(concat('%', :pesquisa, '%')) OR " +
       "UPPER(e.cep) LIKE UPPER(concat('%', :pesquisa, '%'))) " +
       "ORDER BY e.logradouro ASC")
    Page<Endereco> searchByUsuario(@Param("usuario") Usuario usuario, @Param("pesquisa") String pesquisa,
            Pageable pageable);
